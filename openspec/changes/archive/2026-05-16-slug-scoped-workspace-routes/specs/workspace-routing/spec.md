...
2. Obtener la organización por slug vía `auth.api.getFullOrganization({ query: { organizationSlug: slug } })`. Si no existe → `notFound()`.
...
5. Renderizar `<AppShell>` con `sidebarConfig` derivada del rol del usuario en la org.
