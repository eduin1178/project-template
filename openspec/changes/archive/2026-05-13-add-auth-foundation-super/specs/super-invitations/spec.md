...
El sistema SHALL exponer una server action `createSuperInvitation({ email })` que SHALL verificar `auth.api.getSession()` y rechazar si el rol del solicitante NO es `super_admin`. La action SHALL generar un token criptográficamente seguro, persistirlo, y enviar email con enlace `/{baseUrl}/accept-invitation?token={token}`.

#### Scenario: Super_admin invita
- **WHEN** un super_admin autenticado invoca `createSuperInvitation` con un email válido
- **THEN** se crea el registro en `superInvitation`, se envía el email, y la action retorna confirmación
...
### Requirement: Página de aceptación `/accept-invitation`

El sistema SHALL exponer una ruta pública `/accept-invitation?token={token}` que SHALL validar el token (existe, no expirado, no aceptado) y renderizar formulario de signup pre-llenado con `invitedEmail` (campo editable).

#### Scenario: Token válido renderiza formulario
- **WHEN** se navega con un token válido
...
La aceptación de invitación (sea por email/password o Google) SHALL ocurrir en una transacción que: (a) re-valide el token, (b) cree o asocie el `user`, (c) marque `user.role = "super_admin"`, (d) marque la invitación `acceptedAt` y `acceptedBy`. Si cualquier paso falla, todo se revierte.

#### Scenario: Aceptación con email/password
- **WHEN** un usuario envía signup desde la página de aceptación con token válido
- **THEN** en una transacción se crea `user` (con `role = "super_admin"`, `emailVerified: true` si el flujo lo permite o disparando verificación), se marca la invitación como aceptada, y se redirige a `/login` o `/super` según corresponda

#### Scenario: Aceptación con Google
- **WHEN** un usuario completa OAuth de Google desde la página de aceptación (el token se persiste en estado/PKCE/cookie durante el redirect)
