...
- [x] 1.1 En `lib/auth/server.ts`, agregar `user.additionalFields.lastActiveOrganizationId = { type: "string", required: false, input: false }`.
- [x] 1.2 Comentar `import "server-only"` en `server.ts`, `emails.ts`, `db/client.ts`. Ejecutar `npm run db:generate-auth-schema`. Restaurar imports.
- [x] 1.3 Verificar que `user` table en `lib/db/schema/auth.ts` ahora tenga `lastActiveOrganizationId: text("last_active_organization_id")`.
- [x] 1.4 `npm run db:generate` para nueva migración SQL incremental.
- [x] 1.5 (Pendiente para el usuario): `npm run db:push` en su BD local.

## 2. Helpers

- [x] 2.1 En `lib/auth/guards.ts`, agregar `loadActiveOrganizationsFor(userId)` que devuelva `{ id, name, logo, role }[]` joinando `member` con `organization` filtrado por `member.status='active'`. Ordenar por `organization.name`.
- [x] 2.2 En `lib/auth/guards.ts`, agregar `resolveActiveOrganization({ userId, sessionActiveOrgId, lastActiveOrgId, activeOrgs })` que retorne `{ activeOrgId, needsPersist }`. Lógica:
...
- [x] 3.1 Crear `components/layout/team-switcher-actions.ts` con `switchActiveOrganizationAction(orgId)`:
  - `requireSession`.
...
- [x] 4.1 Crear `components/layout/team-switcher.tsx` (client component) con tipo prop `{ orgs, activeOrgId, action }`. Usa `DropdownMenu` de shadcn dentro de `SidebarMenu` siguiendo el patrón de `nav-user.tsx`.
- [x] 4.2 Trigger: `SidebarMenuButton size="lg"` con avatar/iniciales de la org activa (componente `OrgAvatar` ya existe en `components/organizations/`), nombre, y `CaretUpDownIcon` a la derecha.
- [x] 4.3 Content: header "Cambiar de organización", item por cada org (avatar + nombre + check si activa). Click → `useTransition` → llama action → `router.refresh()` + toast on success.

## 5. Sidebar config

- [x] 5.1 En `components/layout/types.ts`, agregar:
  ```ts
  ...
  ```
- [x] 5.2 En `components/layout/app-sidebar.tsx`, aceptar prop opcional `teams?: TeamsConfig`. Si llega → renderizar `<TeamSwitcher>` en el `SidebarHeader`. Si no → renderizar `NavBrand` (comportamiento actual).

## 6. Layouts

- [x] 6.1 `app/admin/layout.tsx`:
...
- [x] 6.2 `app/app/layout.tsx`: idem.
- [x] 6.3 `app/super/(protected)/layout.tsx`: NO cambia (no se pasa `teams`).
