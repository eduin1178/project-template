# no-organization-page Specification

## Purpose

Página informativa `/no-organization` para usuarios autenticados que no pertenecen a ninguna institución activa. Reusa el mismo `AppShell` que las rutas `/account/*` para entregar una experiencia coherente: shell completo con sidebar sin items de workspace, `TeamSwitcher` en estado placeholder y CTAs hacia el flujo de invitaciones y perfil.
