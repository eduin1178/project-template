...
El sistema SHALL renderizar los comentarios en un panel `TaskCommentsPanel` ubicado inline al final del `TaskDetailPane`, debajo del bloque de equipo. El panel SHALL renderizarse para cualquier viewer que tenga acceso al detail pane (la regla de visibilidad de la tarea ya filtra el acceso). El panel SHALL listar los comentarios cronológicamente ASC; cada fila SHALL mostrar avatar del autor, nombre del autor, timestamp relativo (ej. "hace 5 min") y el body. Los comentarios con `deletedAt != NULL` SHALL renderizarse con un placeholder en lugar del body:
...
Al pie del panel SHALL haber un composer con `<Textarea>` multilínea (2 a 3 filas mínimas) y un botón "Enviar". El composer SHALL invocar `createComment` y, al recibir éxito, limpiar el textarea y refrescar la lista. Presionar Enter en el textarea SHALL enviar; presionar Shift+Enter SHALL insertar un salto de línea. El composer SHALL estar deshabilitado si la capability `canComment` del viewer sobre la tarea es false; el panel SHALL ocultar el composer en ese caso. El botón "Eliminar" de cada comentario SHALL renderizarse si y solo si el campo `canDelete` proyectado para esa fila es true; al hacer click SHALL invocar `deleteComment(commentId)` y, al recibir éxito, refrescar la lista. Todo el copy SHALL estar en español neutral en segunda persona singular `tú`, sin voseo.

#### Scenario: Panel de comentarios visible al final del detail pane
- **WHEN** un viewer autorizado selecciona una tarea en el detail pane
- **THEN** el panel de comentarios se renderiza al final del detail pane, debajo del bloque de equipo
...
- **THEN** se invoca `createComment` con el contenido del textarea trimeado; al recibir éxito, el textarea se limpia y la lista se refresca con el nuevo comentario al final
...
- **WHEN** un comentario tiene `deletedAt != NULL` y `deletedByEmail` distinto del email del autor (eliminado por admin)
...
- **WHEN** el autor del comentario hace click en "Eliminar" y la action llega al servidor cuando ya pasaron 60 minutos (botón se renderizó dentro de la ventana pero la red demoró)
...
- **WHEN** se inspecciona el copy visible del panel de comentarios (placeholder del textarea, botón Enviar, botón Eliminar, mensajes de error, placeholder de comentario eliminado)
