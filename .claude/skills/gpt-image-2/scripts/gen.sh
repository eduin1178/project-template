...
if command -v python3 >/dev/null 2>&1 && python3 -c "import sys" >/dev/null 2>&1; then
  ...
  PYTHON_BIN="py -3"
elif [[ -x "/c/Users/$USER/AppData/Local/Programs/Python/Python313/python.exe" ]]; then
  PYTHON_BIN="/c/Users/$USER/AppData/Local/Programs/Python/Python313/python.exe"
elif command -v python >/dev/null 2>&1 && python -c "import sys" >/dev/null 2>&1; then
  PYTHON_BIN="python"
else
  echo "python3 not found (and no usable 'py -3' or 'python' fallback)" >&2; exit 3
fi
...
# Codex 0.128.0+ writes the generated PNG directly to
# ~/.codex/generated_images/<session_id>/<call_id>.png instead of embedding
# base64 in the rollout. Detect that path first; fall back to legacy extractor.
GEN_DIR_ROOT="$HOME/.codex/generated_images"
found_png=""
while IFS= read -r sess_path; do
  [[ -z "$sess_path" ]] && continue
  sid="$(basename "$sess_path" .jsonl)"
  sid="${sid#rollout-*-}"
  candidate_dir="$GEN_DIR_ROOT/$sid"
  if [[ -d "$candidate_dir" ]]; then
    latest_png="$(ls -t "$candidate_dir"/*.png 2>/dev/null | head -n1)"
    if [[ -n "$latest_png" && -f "$latest_png" ]]; then
      found_png="$latest_png"
      break
    fi
  fi
done < "$new_sessions_file"

if [[ -z "$found_png" ]]; then
  # Fallback to scanning generated_images by mtime (covers rename quirks)
  found_png="$(find "$GEN_DIR_ROOT" -type f -name '*.png' -newer "$before" 2>/dev/null | xargs -I {} ls -t {} 2>/dev/null | head -n1)"
fi
...
if [[ -n "$found_png" && -f "$found_png" ]]; then
  ...
  set +e
  $PYTHON_BIN "$SCRIPT_DIR/extract_image.py" "$OUT" "$new_sessions_file"
  py_rc=$?
  set -e
  if [[ $py_rc -ne 0 ]]; then
    echo "Image payload not found in session rollout or generated_images dir" >&2
